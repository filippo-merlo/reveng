...
def create_latex_table():
    ...
    csv_path = os.path.join(
        os.path.dirname(__file__), "outputs", "summary_by_size_complexity.csv"
    )
    ...
    size_recovery = (
        df_for_recovery.groupby("grid_size")["error_recovery_rate"].mean().reset_index()
    )
    ...
    # Calculate recovery only from filtered data
    complexity_recovery = (
        df_for_recovery.groupby("complexity")["error_recovery_rate"]
        .mean()
        .reset_index()
    )
    complexity_table = complexity_table.merge(
        complexity_recovery, on="complexity", how="left"
    )
    complexity_table = complexity_table.rename(columns={"complexity": "Value"})

    # Rename metrics columns
    size_table = size_table.rename(
        columns={
    ...
    complexity_table = complexity_table.rename(
        columns={
            "mean_taken_optimal_gt": "Opt. GT",
            "mean_taken_optimal_decoded": "Opt. Dec.",
            "mean_gt_decoded_agreement": "Agreement",
